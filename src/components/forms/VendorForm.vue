<template>
  ...
  <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑供应商' : '新增供应商'" width="500px" @close="handleClose">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="80px" label-position="top">
      ...
      <el-form-item label="供应商编号/税号" prop="id">
        <el-input v-model="form.id" placeholder="请输入供应商编号或税号" clearable />
      </el-form-item>
...
const emit = defineEmits<{
  ...
  submit: [data: VendorCreatePayload];
}>();
...
const isEdit = computed(() => !!props.vendor?.guid);

const form = reactive<VendorCreatePayload>({
  name: '',
  id: '',
  notes: '',
  active: true,
